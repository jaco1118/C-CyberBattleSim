...
`compute_q1_reconciliation.py`, `q1_run_output.log` (Question Set 1); `rq2b_reproduction/`
(`compute_z_mde.py`, all 12 original preserved per-seed CSVs, `q3_reproduction_output.log`)
(Question Set 3).

## Wipe test
Question Set 1 reproducible from the committed script and the already-on-disk
`attenuation_drift_logs/` (raw data not committed, per convention). Question Set 3's band 10-15
