...
SEED = int(sys.argv[1])
TOPOLOGY_ID = sys.argv[2]           # scalability_80_100/<id>
RUN_FOLDER = os.path.abspath(sys.argv[3])
STEPS_OVERRIDE = int(sys.argv[4]) if len(sys.argv) > 4 else None  # smoke-test only
CONDITION = "static"                # F2 is STATIC only
...
TOPOLOGY_PKL = os.path.join(REPO, f"cyberbattle/data/env_samples/scalability_80_100/{TOPOLOGY_ID}/network_SecureBERT.pkl")
...
cfg["name"] = f"trpo_250k_F2_static_band80-100_seed{SEED}"
cfg["seeds_runs"] = [SEED]
...
cfg["drift_seed"] = SEED
cfg["drift_scenario_id"] = f"scalability_80_100_{TOPOLOGY_ID}"
...
print(f"[{cfg['name']}] device={device}, starting 250000-step TRPO training on topology {TOPOLOGY_ID} "
