...
git stash push -m "taskL-step2det" cyberbattle/_env/cyberbattle_env_compressed.py > "$B/L_regd_stash.log" 2>&1
for BAND in 30-40 80-100; do
  OUT=$B/L_regd_$BAND
  python "$B/run_L_regression.py" old 42 $STEPS $BAND "$OUT" > "$OUT/old.log" 2>&1
done
git stash pop >> "$B/L_regd_stash.log" 2>&1
echo "OLD done + popped $(date +%H:%M:%S); diff: $(git diff --stat cyberbattle/_env/cyberbattle_env_compressed.py | tail -1)"
