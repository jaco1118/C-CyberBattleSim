...
def collect_band_data(band_label, band_config, manifest, logger):
    ...
    # AMENDMENT (Task T STEP 5 revised, option B): a fresh --collect run used to just
    # os.remove() any existing drift_{band}.csv outright -- which is exactly why the original
    # 830-episode single-seed gate's raw CSV no longer exists anywhere. Rename the previous
    # file aside (by its own mtime, so nothing is ever silently clobbered) instead of deleting
    # it. Deliberately the minimal fix: does NOT change the fixed drift_{band}.csv path
    # --analyze-only mode relies on, does NOT add a run-scoped folder or new CLI surface --
    # both were considered and rejected in favour of this smaller change, to avoid re-opening
    # the file-path contract that the aggregation-site audit and the deterministic regression
    # test just finished clearing.
    if os.path.exists(drift_log_path):
        backup_path = os.path.join(
            DRIFT_LOG_DIR, f"drift_{band_label}_backup_{int(os.path.getmtime(drift_log_path))}.csv"
        )
        os.rename(drift_log_path, backup_path)
...
def load_and_filter(drift_log_path, band_label):
    ...
    # AMENDMENT (Task T STEP 5 revised, ACTION 1): event_id is generated by
    # self._drift_event_counter PER ENV INSTANCE (cyberbattle_env_compressed.py), restarting at 0
    # for every one of the 8-topologies x 5-seeds = 40 instances feeding this one band's CSV --
    # raw event_id.nunique() therefore collides across instances and undercounts distinct events.
    # Confirmed on real data: 119 raw-unique values vs 1957 true distinct (seed, scenario_id,
    # event_id) tuples on the 10-15 band, a >16x undercount. Disambiguate by the full
    # (seed, scenario_id, event_id) tuple -- these three columns together are the only thing that
    # uniquely identifies an event across every env instance that ever wrote to this file.
    visibility_stats = {}
    for ct in df['change_type'].dropna().unique():
        ...
        fired_total_events = ct_rows[ct_rows['event_phase'].isin(['fired'])].groupby(['seed', 'scenario_id', 'event_id']).ngroups
        visibility_stats[ct] = {
            ...
            'n_never_attributed': fired_never_attributed.groupby(['seed', 'scenario_id', 'event_id']).ngroups,
